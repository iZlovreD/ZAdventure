...
ret.area['occupied base'] = {
	...
	,Events = {
		...
		,[defines.events.on_biter_base_built] = function(event, ad, game)
			...
			if event.entity.valid and event.entity.force.name == 'ZADV_force' then
				if event.entity.prototype.name == 'biter-spawner' then
					ad.surface.create_entity{name = 'zadv-beast-spawner', position = event.entity.position, force='ZADV_force'}
					event.entity.destroy()
				elseif event.entity.prototype.name == 'spitter-spawner' then
					ad.surface.create_entity{name = 'zadv-zombie-spawner', position = event.entity.position, force='ZADV_force'}
					event.entity.destroy()
				end
			end
		...
		,[defines.events.on_entity_died] = function(event, ad, game)
			...
			if event.entity.force.name == 'ZADV_force' and event.entity.prototype.name == "zadv-roboport" then
