...
ret.area['template'] = {
	...
	,ScriptForEach = function(game, surface, force, area, center, entity, namelist, locstore, areadata) end
	
	,ScriptForAll = function(game, surface, force, area, center, namelist, entitylist, areadata, locstore) end
			...
			elseif ad.total >= 7500 then
				...
				if ad.nextcheck then
					if ad.nextcheck <= event.tick then
						...
						if not ad.tosuply or #ad.tosuply == 0 then
							ad.disabled = true
							return
						end
...
ret.area['garage'] = {
	...
	,ScriptForEach = function(game, surface, force, area, center, entity, namelist, locstore, areadata)
		...
		areadata.cars = areadata.cars or 1 + (ZADV_Rnd() > 600 and 1 or 0)
		
		if entity.prototype.name == "stone-wall"
		or entity.prototype.name == "gate" then
			entity.force = 'enemy'
			entity.health = entity.prototype.max_health * (ZADV_Rnd()/400)
			
			if ZADV_Rnd() < 100 then
				entity.die('neutral')
			else
		...
		elseif entity.prototype.name == "medium-electric-pole" then
				...
				if ZADV_Rnd() > 700 then
				
					local ent = surface.create_entity{name="tank", position=pos, force=force}
					
					if ent.valid then
						
						ent.health = ent.prototype.max_health * (ZADV_Rnd()/900)
						...
						for chance,variant in pairs(variations) do
							if roll >= tonumber(chance) then selct = variant end
						end
		...
		elseif entity.prototype.name == "small-electric-pole" then
			...
			if ZADV_Rnd() > 400 and areadata.cars > 0 then
			
				local ent = surface.create_entity{name="car", position=pos, force=force}
					
				if ent.valid then
						
					ent.health = ent.prototype.max_health * (ZADV_Rnd()/600)
		...
		elseif entity.prototype.name == "iron-chest" then
			...
			local variations = {
				['000'] = {c=200, n=""},
				['100'] = {c=200, n="raw-wood"},
				['200'] = {c=200, n="coal"},
				['400'] = {c=100, n="solid-fuel"},
				['600'] = {c=20, n="rocket-fuel"},
				['825'] = {c=4, n="nuclear-fuel"}
			}
			...
			if selct.n:len() > 0 then
				entity.get_inventory(defines.inventory.chest).insert( {name=selct.n, count=selct.c} )
			end
...
ret.area['danger chest'] = {
	...
	,ScriptForEach = function(game, surface, force, area, center, entity, namelist, locstore, areadata)
		...
		elseif entity.prototype.name == "land-mine" and ZADV_Rnd()  > 600 then
			entity.destroy()
		end
	end
	...
	,ScriptForAll = function(game, surface, force, area, center, namelist, entitylist, areadata, locstore)
		...
		local variant = areadata.variations[math.max(1,ZADV_Rnd() % #areadata.variations)]
		
		if variant[2] < 0 then
			table.insert(areadata.prize, { name=variant[1], count=1 })
			
		elseif variant[2] ~= 0 or (variant[2] == 0 and ZADV_Rnd() % 10 ~= 0) then 
			for i=1, math.min(16, math.max(variant[2], ZADV_Rnd() % variant[3])), 1 do
		...
		if areadata.armorpack[ variant[1] ] then for _,ins in pairs(areadata.armorpack[ variant[1] ]) do
			...
			else table.insert(areadata.prize, {name=ins[3], count= ZADV_Rnd(ins[2])}) end
			
		end end
		
		if ZADV_Rnd() < 150 then
	...
	,Events = {
		...
		[defines.events.on_tick] = function(event, ad, game)
			...
			if ad.looted and not ad.destroyed then
				local ents = ad.surface.find_entities_filtered{area=ad.area,name=ad.search, limit=2}
				for _,ent in pairs(ents) do
					if ent and ent.valid then ent.die('enemy') end
					if ent and ent.valid then ent.destroy('enemy') end
				end
				if not ents or #ents == 0 then
					for _,e in pairs(ad.surface.find_tiles_filtered{area=ad.area, name={'concrete', 'refined-concrete', 'stone-path', 'refined-hazard-concrete-right', 'refined-hazard-concrete-left', 'hazard-concrete-right', 'hazard-concrete-left'}}) do
						if e and e.position then
							local n = ad.surface.get_hidden_tile(e.position)
							if not n then n = 'red-desert-2' end
							ad.surface.set_tiles({{name=n, position=e.position}})
						end
					end
					ad.destroyed = true
				end
	...
	,ScriptForAll = function(game, surface, force, area, center, namelist, entitylist, areadata, locstore)
		...
		for _,entity in pairs(entitylist) do if entity and entity.valid and entity.prototype.name == "iron-chest" then
			...
			if variant[1] == 'power-armor-mk2' then for _,ins in pairs(areadata.armorpack) do
				...
				if ins[1] < 0 then table.insert(areadata.prize, { name=ins[3], count=1 })
				else table.insert(areadata.prize, {name=ins[3], count= ZADV_Rnd(1,ins[2])}) end
	...
	,Events = {
		...
		[defines.events.on_tick] = function(event, ad, game)
			...
			if ad.looted and not ad.destroyed then
				...
				for _,e in pairs(ad.surface.find_tiles_filtered{area=ad.area, name={'zadv-slow-path-30', 'zadv-slow-path-50', 'zadv-slow-path-70'}}) do
					if e and e.position then
						local n = ad.surface.get_hidden_tile(e.position)
						if not n then n = 'red-desert-2' end
						ad.surface.set_tiles({{name=n, position=e.position}})
			...
			for _,player in pairs(game.players) do if player.surface == ad.surface and next then
				...
				if Area.inside(Area.shrink(ad.area, 16), player.position) and not ad.lootarea then
					...
					if smartass and not ad.looted then
						ad.looted = true
						if ad.entity and ad.entity.valid then
							ad.entity.destroy()
						end
						ad.surface.spill_item_stack(ad.entitypos, {name='coal'}, true)
						player.print("Naughty kids get coal candy!", global.ZADV.Color.black)
						next = false
					end
					...
					if next then
						ad.stored_items = ad.entity.get_inventory(defines.inventory.chest).get_item_count()
						ad.lootarea = true
						player.print("Take your prize!", global.ZADV.Color.blue)
						next = false
					end
					
				elseif Area.inside(Area.shrink(ad.area, 7), player.position) and not ad.inside then
					ad.inside = true; player.print("Hey ".. player.name ..", you are so close to the treasure. Hurry up!", global.ZADV.Color.orange)
					
				elseif Area.inside(ad.area, player.position) and not ad.inarea then
					ad.inarea = true; player.print("You enter the Maze. Don't waste your time!", global.ZADV.Color.yellow)
					
				elseif Area.inside(ad.debuffarea, player.position) and not ad.visited then
					ad.visited = true; player.print("You found The Maze!", global.ZADV.Color.green)
				end
...
ret.area['smart lab'] = {
	...
	,messages = {
		{ msg = "Let be the light! Or science?.. Or.. Whatever...", global.ZADV.Color.blue },
		{ msg = "Give me a place to stand and with a lever. I will.. do something.", global.ZADV.Color.blue },
		{ msg = "Fifteen men on a dead man's chest. Yo ho ho and a bottles of science? o_O", global.ZADV.Color.blue },
		{ msg = "Nerds attack!", global.ZADV.Color.blue }
	}
}
end
...
ret.area['random bp'] = {
	...
	,ScriptForEach = function(game, surface, force, area, center, entity, namelist, locstore, areadata)
		...
		if entity and entity.valid and entity.prototype.name == "wooden-chest" then
			...
			local bp = table.deepcopy(areadata[math.max(1,ZADV_Rnd(1,1000)%#areadata)])
			areadata = nil
...
ret.area['occupied base'] = {
	...
	,messages = {
		{ msg = "BEWARE! New threat on horizont.", global.ZADV.Color.orange }
	}
			...
			for _,player in pairs(game.players) do
				if Area.inside(Position.expand_to_area(ad.center, 32), player.position) then
					...
					if #possible_recipes then
						rec = ZADV_Rnd(1,#possible_recipes,game.tick)
					...
					if not ad.disabled and player.force.recipes[rec] then
						...
						player.print({'message.zadv_hey_recipe',player.force.recipes[rec].prototype.localised_name}, global.ZADV.Color.green)
						ad.disabled = true
					end
			...
			for _,player in pairs(game.players) do
				if Area.inside(Position.expand_to_area(ad.center, 32), player.position) then
					...
					if not ad.disabled and player.force.technologies[tech] then
						...
						player.print({'message.zadv_hey_tech', player.force.technologies[tech].prototype.localised_name}, global.ZADV.Color.green)
						ad.disabled = true
					end
			...
			local function ChangeRecipe()
				...
				if IntegrityCheck('chest') then
					...
					if not next then
						...
						ChangeBehaviour(possible_recipes[ZADV_Rnd(1,#possible_recipes,game.tick)])
...
ret.area['exchange post'] = {
	...
	,ScriptForAll = function(game, surface, force, area, center, namelist, entitylist, areadata, locstore)
		...
		areadata.next_appearance = game.tick + ZADV_Rnd(300,108000) -- 108000
	...
	,Events = {
		...
		[defines.events.on_tick] = function(event, ad, game)
			...
			local function setNextAppearence(ticks)
				ad.next_appearance = event.tick + ZADV_Rnd(1,ticks or 108000) -- 108000
			end
				...
				if ad.working_end then
					...
					if ad.working_end > event.tick then
						...
						if not ad.current_price then
							...
							ad.current_price = ad.variations[ZADV_Rnd(1,1000) % #ad.variations + 1]
...
ret.area['Autodrome'] = {
	...
	,Events = {
		...
		[defines.events.on_tick] = function(event, ad, game)
			...
			local function CheckCombinator()
				...
				if not ad.combinator or not ad.combinator.valid or ad.blowItAll then
					...
					for _,e in pairs(ad.surface.find_entities_filtered{area=ad.area, name={"zadv-race-car-yellow","zadv-race-car-green","zadv-race-car-red" }}) do
						if e and e.valid then e.die('enemy') end
					end
					...
					for _,d in pairs(ad.decals) do
						ad.surface.destroy_decoratives(Position.expand_to_area(Position.offset(ad.center, d[1], d[2]), 1))
					end
			...
			local function AddPrize(id, race_result, damage)
				...
				local function setPrize(have, new)
					...
					table.insert(ret, _new[ZADV_Rnd(1,#_new,id)])
			...
			local function CheckRaceProgress()
				...
				if ad.race_in_progress then
					...
					if car.id == 3 then
						local g = ad.surface.find_entities_filtered{name='gate', position=ad.gates.race[ZADV_Rnd(1,#ad.gates.race,event.tick)].pos, limit=1}
					...
					if Area.inside(Area.shrink(ad.podarea, 2), car.ent.position) then
						...
						local dmg = car.ent.health / car.ent.prototype.max_health
						local nm = game.players[ad.player_in_car].name
						
						if dmg <= 0.5 then nm = nm .. ' (damaged)' end
						if dmg >= 1.0 then nm = nm .. ' (flawless)' end
						
						race_time.name = nm
						
						AddPrize(car.id, race_time, dmg)
			...
			if step('pp', 60)	then UpdateRaceGUI() end
		...
		,[defines.events.on_entity_died] = function(event, ad, game)
			...
			if event.entity and event.entity.prototype.name:find('zadv-race-car')
			and tostring(event.entity.effectivity_modifier):find(ad.carmark) then
				ad.ResetRace()
			end
